...
# Gráfico comparativo
ggplot(ipc_edad_region, aes(x = grupos_edad, y = IPC_promedio, group = region, color = region)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
...
ggplot(ipc_region_total, aes(x = grupos_edad, y = IPC_promedio, group = region, color = region)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Comparación del IPC promedio por grupo de edad",
